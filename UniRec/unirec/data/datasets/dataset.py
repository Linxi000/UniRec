...
from unirec.data.splitter import split_data
from unirec.utils import float_dd
...
class Dataset:
    ...
    def _fetch_data(self):
        ...
        self._fetch_item_similarity()
        self._fetch_item_uni()
        self._fetch_user_uni()
        self._fetch_item_head_tail()
        self._fetch_item_neighbors()
        self._fetch_item_variance()
        self.data = {
            'train_set': train_set,
            ...
            'user_uni':self.user_uni,
            'item_uni':self.item_uni,
            'item_head_tail':self.item_head_tail,
            'item_neighbors':self.item_neighbors,
            'item_var':self.item_variance
        }
    ...
    def _fetch_item_similarity(self):
        """
        Fetch item similarity data and store it in a dictionary.
        """
        dataset_name = self.dataset_params['name']
        item_similarity_path = os.path.join(
            self.dataset_params['path'], f'{dataset_name}_item_similarity.pkl')
        if os.path.exists(item_similarity_path):
            with open(item_similarity_path, 'rb') as file:
                loaded_data = pickle.load(file)
                self.item_similarity = {int(key): int(value) for key, value in loaded_data.items()}

            self.logger.info('Item similarity data loaded successfully.')
        else:
            self.logger.error('Item similarity data file not found.')

    def _fetch_item_uni(self):
        """
        Fetch item universality data and store it in a dictionary.
        """
        dataset_name = self.dataset_params['name']
        item_uni_path = os.path.join(
            self.dataset_params['path'], f'{dataset_name}_item_uni.pkl')
        if os.path.exists(item_uni_path):
            with open(item_uni_path, 'rb') as file:
                loaded_data = pickle.load(file)
                self.item_uni = {key: int(value) for key, value in loaded_data.items()}

            self.logger.info('Item uni data loaded successfully.')
        else:
            self.logger.error('Item uni data file not found.')

    def _fetch_user_uni(self):
        """
        Fetch item universality data and store it in a dictionary.
        """
        dataset_name = self.dataset_params['name']
        user_uni_path = os.path.join(
            self.dataset_params['path'], f'{dataset_name}_user_uni.pkl')
        if os.path.exists(user_uni_path):
            with open(user_uni_path, 'rb') as file:
                self.user_uni = pickle.load(file)
            self.logger.info('User uni data loaded successfully.')
        else:
            self.logger.error('User uni data file not found.')

    def _fetch_item_head_tail(self):
        """
        Fetch item head-tail data and store it in a dictionary.
        """
        dataset_name = self.dataset_params['name']
        item_head_tail_path = os.path.join(
            self.dataset_params['path'], f'{dataset_name}_item_head_tail.pkl')
        if os.path.exists(item_head_tail_path):
            with open(item_head_tail_path, 'rb') as file:
                self.item_head_tail = pickle.load(file)
            self.logger.info('Item head-tail data loaded successfully.')
        else:
            self.logger.error('Item head-tail data file not found.')

    def _fetch_item_neighbors(self):
        """
        Fetch item neighbors data and store it in a dictionary.
        """
        dataset_name = self.dataset_params['name']
        item_neighbors_path = os.path.join(
            self.dataset_params['path'], f'{dataset_name}_item_neighbors.pkl')
        if os.path.exists(item_neighbors_path):
            with open(item_neighbors_path, 'rb') as file:
                loaded_data = pickle.load(file)
                self.item_neighbors = {int(key): [int(value) for value in values] for key, values in loaded_data.items()}

            self.logger.info('Item neighbors data loaded successfully.')
        else:
            self.logger.error('Item neighbors data file not found.')

    def _fetch_item_variance(self):
        """
        Fetch item variance data and store it in a dictionary.
        """
        dataset_name = self.dataset_params['name']
        item_variance_path = os.path.join(
            self.dataset_params['path'], f'{dataset_name}_item_var.pkl')
        if os.path.exists(item_variance_path):
            with open(item_variance_path, 'rb') as file:
                self.item_variance = pickle.load(file)
            self.logger.info('Item variance data loaded successfully.')
        else:
            self.logger.error('Item variance data file not found.')
