...
def entrypoint(params):
    """ Command entrypoint
    :param params: Deserialized JSON configuration file将json配置文件中的内容通过反序列化转换为params字典。
    ...
    with tf.compat.v1.Session(config=sess_config) as sess:
        ...
        for step, seed in enumerate(random_seeds):
            ...
            message = [f'Step #{step + 1}',
                       f'NDCG {score[0]:8.5f}, ', f'HR {score[1]:8.5f}',f'MRR {score[2]:8.5f}'
                       ]
            logger.info(','.join(message))
            scores.append(score)
        ndcg, hr, mrr = zip(*scores)
        message = ['RESULTS:',
                   f'NDCG: {np.mean(ndcg)+np.std(ndcg):8.5f}',
                   f'HR: {np.mean(hr)+np.std(hr):8.5f}',
                   f'MRR: {np.mean(mrr)+np.std(mrr):8.5f}'
                   ]
        print('RESULTS:',
                   f'NDCG: {np.mean(ndcg)+np.std(ndcg):8.5f}',
                   f'HR: {np.mean(hr)+np.std(hr):8.5f}',
