...
from unirec.models.abs.seq import AbsSeqRec
...
class AbsSasRec(AbsSeqRec):
    ...
    def _learnable_abs_position_embedding(self, seq , position_embedding_table):
        """
        ...
        position_ids = tf.tile(
            tf.expand_dims(tf.range(tf.shape(seq)[1]), 0),
            [tf.shape(seq)[0], 1])
