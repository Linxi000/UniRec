...
from unirec.data.loaders import dataloader_factory
from unirec.eval.evaluator import Evaluator
...
class Trainer:
    ...
    def __init__(self, sess, model, params):
        ...
        self.n_epochs = self.params['training'].get('num_epochs', 200)
    ...
    def fit(self, data):
        ...
        sim_path=dataset_params['path']
        data_name=dataset_params['name']
        ...
        with open(metrics_path, 'w') as f:
            ...
            for ep in range(self.n_epochs):
                ...
                if epochs_since_improvement >= 40:
                    self.logger.info(
                        f'Training stopped early after {ep + 1} epochs due to no improvement over the last 30 epochs.')
                    break
    ...
    def _get_epoch_loss(self, dataloader, epoch_id):
        ...
        for _ in tqdm(range(1, n_batches), desc=f'{desc}...'):
            ...
            batch_loss, batch_reg_loss = self._get_batch_loss(batch=batch_data, epoch_id=epoch_id)
    ...
    def _get_batch_loss(self, batch, epoch_id):
        """
        Forward pass for a batch
        :param batch:
        :return:
        """
        feed_dict = self.model.build_feedict(batch, epoch_num=epoch_id, is_training=True)

        reg_loss = 0.0
        _, loss, _ = self.sess.run(
            [self.model.train_ops, self.model.loss,  self.model.global_step],
            feed_dict=feed_dict)
        return loss, reg_loss

    def batch_loss_choose(self, current_epoch):
